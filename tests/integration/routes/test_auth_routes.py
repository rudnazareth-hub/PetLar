...
class TestLogin:
    ...
    def test_usuario_logado_nao_acessa_login(self, adotante_autenticado):
        """Usuário já logado deve ser redirecionado ao acessar /login"""
        response = adotante_autenticado.get("/login", follow_redirects=False)
        assert_redirects_to(response, "/usuario")
...
class TestLogout:
    ...
    def test_logout_limpa_sessao(self, adotante_autenticado):
        """Logout deve limpar sessão e redirecionar para raiz"""
        response = adotante_autenticado.get("/logout", follow_redirects=False)

        # Deve redirecionar para raiz do site
        assert_redirects_to(response, "/")

    def test_logout_desautentica_usuario(self, adotante_autenticado):
        """Após logout, usuário não deve ter acesso a áreas protegidas"""
        # Fazer logout
        adotante_autenticado.get("/logout")

        # Tentar acessar área protegida
        response = adotante_autenticado.get("/chamados/listar", follow_redirects=False)
...
class TestAutorizacao:
    ...
    def test_usuario_autenticado_acessa_area_protegida(self, adotante_autenticado):
        """Usuário autenticado deve acessar áreas protegidas"""
        response = adotante_autenticado.get("/chamados/listar")
        assert response.status_code == status.HTTP_200_OK

    def test_adotante_nao_acessa_area_admin(self, adotante_autenticado):
        """Adotante não deve acessar áreas administrativas"""
...
class TestCadastroAdicional:
    ...
    def test_usuario_logado_nao_acessa_cadastro(self, adotante_autenticado):
        """Usuário já logado deve ser redirecionado ao acessar /cadastrar"""
        response = adotante_autenticado.get("/cadastrar", follow_redirects=False)
        assert_redirects_to(response, "/usuario")
