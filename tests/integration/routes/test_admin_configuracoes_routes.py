...
class TestTema:
    ...
    def test_get_tema_requer_admin(self, adotante_autenticado):
        """Adotante não deve acessar seletor de temas"""
        response = adotante_autenticado.get("/admin/tema", follow_redirects=False)
    ...
    def test_adotante_nao_pode_aplicar_tema(self, adotante_autenticado):
        """Adotante não deve poder aplicar tema"""
...
class TestAuditoria:
    ...
    def test_get_auditoria_requer_admin(self, adotante_autenticado):
        """Adotante não deve acessar auditoria"""
        response = adotante_autenticado.get("/admin/auditoria", follow_redirects=False)
        assert response.status_code in [
            status.HTTP_303_SEE_OTHER,
            status.HTTP_403_FORBIDDEN,
        ]
    ...
    def test_adotante_nao_pode_filtrar_logs(self, adotante_autenticado):
        """Adotante não deve poder filtrar logs"""
        data_hoje = agora().strftime("%Y-%m-%d")
...
class TestSegurancaConfiguracoes:
    ...
    def test_abrigo_nao_acessa_tema(self, abrigo_autenticado):
        """Abrigo não deve acessar temas"""
        response = abrigo_autenticado.get("/admin/tema", follow_redirects=False)
        assert response.status_code in [
            status.HTTP_303_SEE_OTHER,
            status.HTTP_403_FORBIDDEN,
        ]

    def test_abrigo_nao_acessa_auditoria(self, abrigo_autenticado):
        """Abrigo não deve acessar auditoria"""
        response = abrigo_autenticado.get("/admin/auditoria", follow_redirects=False)
        assert response.status_code in [
            status.HTTP_303_SEE_OTHER,
            status.HTTP_403_FORBIDDEN,
        ]
