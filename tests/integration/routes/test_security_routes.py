...
class TestEscalacaoPrivilegios:
    ...
    def test_adotante_nao_pode_acessar_admin_usuarios(self, adotante_autenticado):
        """Adotante não deve acessar painel de administração de usuários"""
    ...
    def test_adotante_nao_pode_criar_usuario_como_admin(self, adotante_autenticado):
        """Adotante não deve poder acessar endpoint de criação de usuário admin"""
    ...
    def test_abrigo_nao_pode_acessar_backups(self, abrigo_autenticado):
        """Abrigo não deve acessar área de backups (apenas admin)"""
    ...
    def test_abrigo_nao_pode_criar_backup(self, abrigo_autenticado):
        """Abrigo não deve poder criar backups"""
...
class TestAutorizacaoArquivos:
    ...
    def test_usuario_nao_pode_baixar_backup_diretamente(
        ...
        # Este teste assume que adotante_autenticado não é admin
...
class TestSessionSecurity:
    ...
    def test_logout_invalida_sessao(self, adotante_autenticado):
        """Logout deve invalidar sessão e prevenir acesso subsequente"""

        # Verificar que está logado antes do logout
        response_antes = adotante_autenticado.get("/usuario", follow_redirects=False)
        assert (
            response_antes.status_code == 200
        ), "Deveria estar autenticado antes do logout"

        # Fazer logout
        response_logout = adotante_autenticado.get("/logout", follow_redirects=False)
        assert_redirects_to(response_logout, "/")

        # Tentar acessar área protegida com mesma sessão
        response_protegido = adotante_autenticado.get(
            "/usuario", follow_redirects=False
        )
